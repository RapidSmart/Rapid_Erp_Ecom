...
function CountryPagination({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
      <div
        role="group"
        aria-label={t('country.pagination.pageSizeLabel')}
        className="flex items-center gap-1"
      >
        {COUNTRY_PAGE_SIZES.map((size) => (
          <button
            key={size}
            type="button"
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          aria-label={t('country.pagination.first')}
        ...
        <button
          ...
          disabled={isFirstPage}
          aria-label={t('country.pagination.previous')}
          className={navButtonClasses}
        >
        ...
        <span className="flex h-7 items-center gap-1 px-1 text-[11px] font-medium text-ink">
          ...
          {t('country.pagination.ofPages', { count: pageCount })}
        </span>
        ...
        <button
          ...
          onClick={() => onPageChange(page + 1)}
          disabled={isLastPage}
          aria-label={t('country.pagination.next')}
          className={navButtonClasses}
        >
        ...
        <button
          ...
          disabled={isLastPage}
          aria-label={t('country.pagination.last')}
          className={navButtonClasses}
        >
