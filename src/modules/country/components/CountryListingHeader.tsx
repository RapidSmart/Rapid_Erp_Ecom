  ...
  return (
    <div className="flex flex-wrap items-center gap-3 sm:flex-nowrap sm:gap-4">
      <button
        ...
        aria-label={t('country.listing.menu')}
      ...
      <div className="flex flex-col gap-0.5">
        {/*
          Not a real <h1>: app.css keeps unlayered legacy hero styles
          (font-size: 56px; margin: 32px 0) that beat any Tailwind text-*
          utility for h1/h2 elements. role="heading" keeps the a11y
          semantics without inheriting that global rule.
        */}
        <p
          role="heading"
          aria-level={1}
        ...
        >
          {t('country.listing.title')}
        </p>
        <span className="truncate text-[11px] leading-none text-ink-subtle">
          {t('country.listing.subtitle', { count: totalCount })}
        </span>
      </div>
      ...
      <div className="order-last flex w-full justify-center sm:order-none sm:w-auto sm:flex-1">
        <label className="flex h-7 w-full max-w-60 items-center gap-2 rounded-full bg-surface-muted px-3">
          ...
          <span className="sr-only">{t('country.listing.searchLabel')}</span>
          <input
            type="search"
            value={search}
            placeholder={t('country.listing.searchPlaceholder')}
