...
import { SubCategoryOverviewRing } from './SubCategoryOverviewRing'
import type { SubCategoryOverviewCardProps } from '../types/sub-category.types'

function SubCategoryOverviewCard({
  status,
  label,
  value,
  percentage,
  ...
  actionLabel,
  onSelect,
}: SubCategoryOverviewCardProps) {
  const { t } = useTranslation()

  const content = (
    <>
      <SubCategoryOverviewRing status={status} percentage={percentage} />
  ...
  return (
    <button
      ...
      aria-label={
        actionLabel ?? t('subCategory.overview.statFilterHint', { status: label })
      }
      style={{
        ...backgroundStyle,
...
export { SubCategoryOverviewCard }
