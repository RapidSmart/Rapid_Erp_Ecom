...
import { SubCategoryViewToggle } from './SubCategoryViewToggle'
import type { SubCategoryListingHeaderProps } from '../types/sub-category.types'

function SubCategoryListingHeader({
  search,
  onSearchChange,
  onToggleMenu,
  totalCount,
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 sm:flex-nowrap sm:gap-4">
      <button
        ...
        aria-label={t('subCategory.listing.menu')}
      ...
      <div className="flex flex-col gap-0.5">
        ...
        >
          {t('subCategory.listing.title')}
        </p>
        <span className="truncate text-[11px] leading-none text-ink-subtle">
          {t('subCategory.listing.subtitle', { count: totalCount })}
        </span>
      </div>

      <SubCategoryViewToggle view={view} onToggle={onToggleView} />
      ...
      <div className="order-last flex w-full justify-center sm:order-none sm:w-auto sm:flex-1">
        <label className="flex h-7 w-full max-w-60 items-center gap-2 rounded-full bg-surface-muted px-3">
          ...
          <span className="sr-only">{t('subCategory.listing.searchLabel')}</span>
          <input
            type="search"
            value={search}
            placeholder={t('subCategory.listing.searchPlaceholder')}
      ...
      >
        {t('subCategory.listing.add')}
      </Link>
    </div>
  )
}

export { SubCategoryListingHeader }
