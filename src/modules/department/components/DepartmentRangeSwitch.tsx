...
import { DEPARTMENT_TIME_RANGES } from '../constants/department-overview.data'
import type { DepartmentTimeRange } from '../types/department.types'
...
function DepartmentRangeSwitch({ range, onRangeChange }: DepartmentRangeSwitchProps) {
  const { t } = useTranslation()

  return (
    <div
      role="group"
      aria-label={t('department.overview.rangeLabel')}
      className="flex flex-wrap items-center gap-1.5 rounded-xl border border-surface-border p-1"
    >
      {DEPARTMENT_TIME_RANGES.map((option) => {
