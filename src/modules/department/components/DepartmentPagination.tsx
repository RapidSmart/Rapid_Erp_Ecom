...
import { DEPARTMENT_PAGE_SIZES } from '../constants/department-pagination.data'
import type { DepartmentPaginationProps } from '../types/department.types'
...
function DepartmentPagination({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
      <div
        role="group"
        aria-label={t('department.pagination.pageSizeLabel')}
        className="flex items-center gap-1"
      >
        {DEPARTMENT_PAGE_SIZES.map((size) => (
          <button
            key={size}
            type="button"
      ...
      <p className="text-[11px] text-ink-subtle" aria-live="polite">
        {t('department.pagination.showing', {
          from: rangeStart,
          to: rangeEnd,
          total: totalCount,
        })}
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          aria-label={t('department.pagination.first')}
        ...
        <button
          ...
          disabled={isFirstPage}
          aria-label={t('department.pagination.previous')}
          className={navButtonClasses}
        >
        ...
        <span className="flex h-7 items-center gap-1 px-1 text-[11px] font-medium text-ink">
          ...
          {t('department.pagination.ofPages', { count: pageCount })}
        </span>
        ...
        <button
          ...
          onClick={() => onPageChange(page + 1)}
          disabled={isLastPage}
          aria-label={t('department.pagination.next')}
          className={navButtonClasses}
        >
        ...
        <button
          ...
          disabled={isLastPage}
          aria-label={t('department.pagination.last')}
          className={navButtonClasses}
        >
