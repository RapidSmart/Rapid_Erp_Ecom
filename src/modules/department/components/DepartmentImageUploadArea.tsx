...
import type { DepartmentImageUploadAreaProps } from '../types/department.types'
import { useDepartmentImageUpload } from '../hooks/useDepartmentImageUpload'
import { Upload, X } from 'lucide-react'

export function DepartmentImageUploadArea({
  imageFile,
  selectedImage,
  imageGallery,
  uploadText,
  ...
  onDragOver,
  onDrop,
}: DepartmentImageUploadAreaProps) {
  const {
    fileRef,
    objectUrl,
    selectedGalleryItem,
  ...
  return (
    <>
      <div
        role="region"
        aria-label="Department image upload area"
      ...
      >
        {hasUploadedFile ? (
          ...
          <button
            ...
            aria-label="Upload department image — drag and drop or click to browse"
      ...
      <input
        ...
        type="file"
        accept="image/*"
        aria-label="Choose department image file to upload"
        className="hidden"
        onChange={(e) => {
