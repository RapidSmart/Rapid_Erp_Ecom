...
import { PRODUCT_TIME_RANGES } from '../constants/product-overview.data'
import type { ProductTimeRange } from '../types/product.types'

export interface ProductRangeSwitchProps {
  range: ProductTimeRange
  onRangeChange: (range: ProductTimeRange) => void
}

function ProductRangeSwitch({ range, onRangeChange }: ProductRangeSwitchProps) {
  const { t } = useTranslation()

  return (
    <div
      role="group"
      aria-label={t('product.overview.rangeLabel')}
      className="flex flex-wrap items-center gap-1.5 rounded-xl border border-surface-border p-1"
    >
      {PRODUCT_TIME_RANGES.map((option) => {
