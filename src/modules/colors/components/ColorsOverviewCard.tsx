...
import { ColorOverviewRing } from './ColorOverviewRing'
import type { ColorStatus } from '../types/colors.types'

export interface ColorsOverviewCardProps {
  status: ColorStatus
  label: string
  value: number
  percentage: number
...
function ColorsOverviewCard({
  ...
  actionLabel,
  onSelect,
}: ColorsOverviewCardProps) {
  const { t } = useTranslation()

  const content = (
    <>
      <ColorOverviewRing status={status} percentage={percentage} />
  ...
  return (
    <button
      ...
      aria-label={
        actionLabel ?? t('colors.overview.statFilterHint', { status: label })
      }
      style={{
        ...backgroundStyle,
