...
import { COLOR_PAGE_SIZES } from '../constants/colors-pagination.data'
import type { ColorPageSize } from '../types/colors.types'
...
function ColorsPagination({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
      <div
        role="group"
        aria-label={t('colors.pagination.pageSizeLabel')}
        className="flex items-center gap-1"
      >
        {COLOR_PAGE_SIZES.map((size) => (
          <button
            key={size}
            type="button"
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          aria-label={t('colors.pagination.first')}
        ...
        <button
          ...
          disabled={isFirstPage}
          aria-label={t('colors.pagination.previous')}
          className={navButtonClasses}
        >
        ...
        <span className="flex h-7 items-center gap-1 px-1 text-[11px] font-medium text-ink">
          ...
          {t('colors.pagination.ofPages', { count: pageCount })}
        </span>
        ...
        <button
          ...
          onClick={() => onPageChange(page + 1)}
          disabled={isLastPage}
          aria-label={t('colors.pagination.next')}
          className={navButtonClasses}
        >
        ...
        <button
          ...
          disabled={isLastPage}
          aria-label={t('colors.pagination.last')}
          className={navButtonClasses}
        >
