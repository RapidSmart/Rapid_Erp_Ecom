...
import type { ColorStatTone } from '../types/colors.types'
...
/** Gauge geometry: the track opens up at the bottom, under the percentage. */
const ARC_START_DEGREES = 117.5
const ARC_SWEEP_DEGREES = 305

const toneTextClasses: Record<ColorStatTone, string> = {
...
export interface ColorStatDonutProps {
  tone: ColorStatTone
  label: string
  value: number
  percentage: number
...
function ColorStatDonut({
  ...
  const arcLength = (clamped / 100) * trackArcLength
  ...
  const content = (
    <>
      ...
      <span
        className={cn(
          'absolute inset-x-0 bottom-1.5 text-center text-[11px] font-semibold',
          toneText
        )}
      >
