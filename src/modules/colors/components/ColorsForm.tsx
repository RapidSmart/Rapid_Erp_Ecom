...
import type { Color, ColorPayload } from "../types/colors.types";

export interface ColorsFormProps {
  color?: Color;
  onSubmit?: (payload: ColorPayload) => void;
  submitting?: boolean;
}

export function ColorsForm({ color, onSubmit, submitting }: ColorsFormProps) {
      ...
      <form noValidate onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
        ...
        <section aria-label="Identity" className="mb-5 sm:mb-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-[1fr_2fr]">
            <PillInput
              id="color-code"
              placeholder={t("colors.form.codePlaceholder") || "e.g. TECH"}
              value={code}
              onChange={setCode}
              maxLength={10}
              required
            ...
            <PillInput
              id="color-name"
              placeholder={t("colors.form.namePlaceholder") || "e.g. Technology"}
              value={name}
              onChange={setName}
              maxLength={40}
              required
        ...
        <section aria-label="Image and Description" className="mb-5 sm:mb-6">
          ...
          <div className="mt-4">
            <label htmlFor="description" className="sr-only">
              {t("colors.form.descriptionPlaceholder") || "Description"}
            </label>
            <textarea
              id="description"
              rows={4}
              maxLength={200}
              placeholder={t("colors.form.descriptionPlaceholder") || "Description"}
              value={description}
        ...
        <FormFooter
          ...
          duplicateText={t("colors.form.submitDuplicate") || "Duplicate"}
          printText="Print"
          clearText="Clear"
          saveText={submitting ? t("colors.form.submitting") || "Saving..." : (t("colors.form.submitCreate") || "Save color")}
          onDuplicate={() => {}}
          onPrint={() => {}}
          onClear={() => {
            setCode("");
