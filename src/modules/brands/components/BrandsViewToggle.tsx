...
export interface BrandsViewToggleProps {
  view: BrandView
  onToggle: () => void
}

function BrandsViewToggle({ view, onToggle }: BrandsViewToggleProps) {
  const { t } = useTranslation()
  const target: BrandView = view === 'list' ? 'grid' : 'list'
  ...
  return (
    <button
      type="button"
      onClick={onToggle}
      aria-label={t('brands.listing.viewToggle', {
        view: t(`brands.listing.view.${target}`),
      })}
    ...
    >
      ...
      {t(`brands.listing.view.${target}`)}
    </button>
  )
}

export { BrandsViewToggle }
