...
import { IndustriesViewToggle } from './IndustriesViewToggle'
import type { IndustryView } from '../types/industries.types'

export interface IndustriesListingHeaderProps {
  search: string
...
function IndustriesListingHeader({
  search,
  onSearchChange,
  onToggleMenu,
  onAdd,
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 sm:flex-nowrap sm:gap-4">
      <button
        ...
        aria-label={t('industries.listing.menu')}
      ...
      <div className="flex flex-col gap-0.5">
        ...
        >
          {t('industries.listing.title')}
        </p>
        <span className="truncate text-[11px] leading-none text-ink-subtle">
          {t('industries.listing.subtitle', { count: totalCount })}
        </span>
      </div>

      <IndustriesViewToggle view={view} onToggle={onToggleView} />
      ...
      <div className="order-last flex w-full justify-center sm:order-none sm:w-auto sm:flex-1">
        <label className="flex h-7 w-full max-w-60 items-center gap-2 rounded-full bg-surface-muted px-3">
          ...
          <span className="sr-only">{t('industries.listing.searchLabel')}</span>
          <input
            type="search"
            value={search}
            placeholder={t('industries.listing.searchPlaceholder')}
      ...
      >
        {t('industries.listing.add')}
      </Button>
    </div>
  )
}

export { IndustriesListingHeader }
