...
import { RakViewToggle } from './RakViewToggle'
import type { RakView } from '../types/rak.types'

export interface RakListingHeaderProps {
  search: string
    ...
    <div className="flex flex-wrap items-center gap-3 sm:flex-nowrap sm:gap-4">
      ...
      <div className="flex flex-col gap-0.5">
        ...
        <span className="truncate text-[11px] leading-none text-ink-subtle">
          {t('rak.listing.subtitle', { count: totalCount })}
        </span>
      </div>

      <RakViewToggle view={view} onToggle={onToggleView} />
      ...
      <div className="order-last flex w-full justify-center sm:order-none sm:w-auto sm:flex-1">
        <label className="flex h-7 w-full max-w-60 items-center gap-2 rounded-full bg-surface-muted px-3">
          ...
          <span className="sr-only">{t('rak.listing.searchLabel')}</span>
          <input
            type="search"
            value={search}
            placeholder={t('rak.listing.searchPlaceholder')}
