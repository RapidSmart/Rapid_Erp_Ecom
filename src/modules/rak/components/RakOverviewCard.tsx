...
import { RakOverviewRing } from './RakOverviewRing'
import type { RakStatus } from '../types/rak.types'

export interface RakOverviewCardProps {
  status: RakStatus
  label: string
  value: number
  percentage: number
...
function RakOverviewCard({
  ...
  actionLabel,
  onSelect,
}: RakOverviewCardProps) {
  const { t } = useTranslation()

  const content = (
    <>
      <RakOverviewRing status={status} percentage={percentage} />
  ...
  return (
    <button
      ...
      aria-label={
        actionLabel ?? t('rak.overview.statFilterHint', { status: label })
      }
      style={{
        ...backgroundStyle,
