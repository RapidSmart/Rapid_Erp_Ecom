...
import type { Rak, RakPayload } from "../types/rak.types";

export interface RakFormProps {
  rak?: Rak;
  onSubmit?: (payload: RakPayload) => void;
  submitting?: boolean;
}

export function RakForm({ rak, onSubmit, submitting }: RakFormProps) {
  const { t } = useTranslation();
  
  const [code, setCode] = useState(rak?.code || "");
  const [name, setName] = useState(rak?.name || "");
  const [description, setDescription] = useState(rak?.description || "");
  const [image, setImage] = useState(rak?.image || "");
      ...
      <form noValidate onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
        ...
        <section aria-label="Identity" className="mb-5 sm:mb-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-[1fr_2fr]">
            <PillInput
              id="rak-code"
              placeholder={t("rak.form.codePlaceholder") || "e.g. TECH"}
              value={code}
              onChange={setCode}
              maxLength={10}
              required
            ...
            <PillInput
              id="rak-name"
              placeholder={t("rak.form.namePlaceholder") || "e.g. Technology"}
              value={name}
              onChange={setName}
              maxLength={40}
              required
        ...
        <section aria-label="Image and Description" className="mb-5 sm:mb-6">
          ...
          <div className="mt-4">
            <label htmlFor="description" className="sr-only">
              {t("rak.form.descriptionPlaceholder") || "Description"}
            </label>
            <textarea
              id="description"
              rows={4}
        ...
        <FormFooter
          ...
          duplicateText={t("rak.form.submitDuplicate") || "Duplicate"}
          printText="Print"
          clearText="Clear"
          saveText={submitting ? t("rak.form.submitting") || "Saving..." : (t("rak.form.submitCreate") || "Save rak")}
          onDuplicate={() => {}}
          onPrint={() => {}}
          onClear={() => {
            setCode("");
