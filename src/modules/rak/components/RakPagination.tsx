...
import { RAK_PAGE_SIZES } from '../constants/rak-pagination.data'
import type { RakPageSize } from '../types/rak.types'

export interface RakPaginationProps {
  page: number
  pageCount: number
...
function RakPagination({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
      <div
        role="group"
        aria-label={t('rak.pagination.pageSizeLabel')}
        className="flex items-center gap-1"
      >
        {RAK_PAGE_SIZES.map((size) => (
          <button
            key={size}
            type="button"
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          aria-label={t('rak.pagination.first')}
        ...
        <button
          ...
          disabled={isFirstPage}
          aria-label={t('rak.pagination.previous')}
          className={navButtonClasses}
        >
        ...
        <span className="flex h-7 items-center gap-1 px-1 text-[11px] font-medium text-ink">
          ...
          {t('rak.pagination.ofPages', { count: pageCount })}
        </span>
        ...
        <button
          ...
          onClick={() => onPageChange(page + 1)}
          disabled={isLastPage}
          aria-label={t('rak.pagination.next')}
          className={navButtonClasses}
        >
        ...
        <button
          ...
          disabled={isLastPage}
          aria-label={t('rak.pagination.last')}
          className={navButtonClasses}
        >
